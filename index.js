...
import pg from 'pg';
import { PluginInterface } from '@beardyman/forge';
 ...
 */
export default class PostgresPlugin extends PluginInterface {
  constructor( config ) {
    super( config );
  ...
  #createColumnDefinitions( columnDefinitions ) {
    return utils.map(columnDefinitions, ({ name, datatype }) => `${name} ${datatype}` ).join( ',\n' );
  }
  ...
  #createColumnValues( columnValues ) {
    return utils.reduce(columnValues, ( result, value, field ) => {
